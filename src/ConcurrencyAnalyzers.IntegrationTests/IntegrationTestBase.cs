...
public class IntegrationTestBase : RedirectingConsoleTest
{
    ...
    protected void LogToOutput(ParallelThreads parallelThreads, bool renderRawStackFrames = false)
    {
        var consoleRenderer = new ConsoleRenderer(renderRawStackFrames);
        consoleRenderer.Render(parallelThreads);
    }

    private async Task GenerateDumpFileIfNeeded(string dumpPath, Action<CancellationToken> stateEstablishing)
    {
    ...
    protected async Task<ParallelThreads> AnalyzeForCurrentProcessAsync(
        Action<CancellationToken> stateEstablishing)
    {
        var cts = new CancellationTokenSource(TimeSpan.FromSeconds(1));
